  ...
  <div id="home">
    ...
    <div style="margin-left: auto; margin-right: 0">
      ...
      <p>
        <small
          >{{
            new Date(newsArticle.creationTime).toLocaleString().split(",")[0]
          }}, Views: {{ newsArticle.views }}</small
        >
      </p>
      <p>
        Tags:
        <small style="color: blue" v-for="tag in tagsByNewsId" :key="tag.id">
          <a @click="getNewsWithTag(tag)"
            ><strong>{{ tag.name }}&nbsp;</strong></a
          >
        </small>
      </p>
    </div>
    <hr />
    <b-button
      v-if="user.id === newsArticle.author || user.id === 1"
      class="btn btn-primary"
      @click="editNews(newsArticle)"
      >Edit</b-button
    >
    |
    <b-button
      ...
      @click="deleteNews(newsArticle)"
      >Delete</b-button
    >
    <br /><br />
    <p class="content">{{ newsArticle.content }}</p>
    <br />
    <a class="btn btn-success" @click="likeNews()"
      ><i class="fa-solid fa-thumbs-up"> {{ newsArticle.likes }}</i></a
    >
    <a class="btn btn-danger" @click="dislikeNews()"
      ><i class="fa-solid fa-thumbs-down"> {{ newsArticle.dislikes }}</i></a
    >
    <br /><br />
    <div>
      <b-button v-b-toggle.sidebar-right variant="info"
        >Articles with similar tags</b-button
      >
          ...
          <div class="col-sm-8">
            ...
            <div v-for="comment in comments" :key="comment.id" id="comments">
              <div class="media comment">
                ...
                <h4 class="media-heading" style="padding-right: 5px">
                  {{ comment.author }}
                </h4>
                ...
                ></ul>
                <a class="btn btn-success" @click="likeComment(comment)"
                  ><i class="fa-solid fa-thumbs-up"> {{ comment.likes }}</i></a
                >
                <a class="btn btn-danger" @click="dislikeComment(comment)"
                ...
                <a
                  ...
                  v-if="
                    (comment.author !== 'Anonymous') &
                      (comment.author === user.name) || user.id === 1
                  "
