...
export const useNewsStore = defineStore({
    ...
    actions: {
        getNews(currentPage, perPage) {
            api.getNews(currentPage, perPage)
            .then(response => {
        ...
        getNewsByCategory(categoryID, currentPage, perPage) {
            api.getNewsByCategory(categoryID, currentPage, perPage)
            .then(response => {
                this.news = response.data;
            })
            .catch(error => {
        ...
        getNewsWithTag(tagID, currentPage, perPage) {
            api.getNewsWithTag(tagID, currentPage, perPage)
            .then(response => {
                this.news = response.data;
            })
            .catch(error => {
        ...
        getSimilarNews(id, currentPage, perPage) {
            api.getSimilarNews(id, currentPage, perPage)
            .then(response => {
        ...
        searchNews(search, currentPage, perPage) {
            api.searchNews(search, currentPage, perPage)
            .then(response => {
                this.news = response.data;
            })
            .catch(error => {
        ...
        updateNews(article) {
            api.updateNews(article.id, article)
            .then(response => {
        ...
        updateNewsLikes(id) {
            ...
            } else if (!this.likedNews.includes(id) && this.dislikedNews.includes(id)) {
                api.likeAfterDislike(id)
                .then(response => {
                    this.likedNews.push(id);
