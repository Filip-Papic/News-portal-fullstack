...
export const useCommentsStore = defineStore({
  ...
  actions: {
    ...
    updateCommentLikes(id) {
      if (
        !this.likedComments.includes(id) &&
        !this.dislikedComments.includes(id)
      ) {
        api
          .likeComment(id)
          .then((response) => {
    ...
    updateCommentDislikes(id) {
      if (
        ...
        api.dislikeAfterLikeComment(id).then((response) => {
          this.comments.forEach((comment) => {
            if (comment.id === id) {
              comment.likes = response.data.likes;
              comment.dislikes = response.data.dislikes;
    ...
    deleteComment(id) {
      api
        ...
        .then(
          ...
          (this.dislikedComments = this.dislikedComments.filter(
            (comment) => comment !== id
          ))
        )
        .catch((error) => {
